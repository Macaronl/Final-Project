# Author: Jermaine Bentley
# Date written: 07/22/2023
# Assignment: Final project
# Short Desc: This program is a quote generator and a tic tac toe game. Each program works precisely as intended.
...
class QuoteGenerator:
 ...
 # Load and set the background image for the quote generator window
        background_image = Image.open("placeholder_image2.jpg")
        background_image = background_image.resize((400, 400), Image.LANCZOS)
        self.background_photo = ImageTk.PhotoImage(background_image)
        self.background_label = tk.Label(self.quote_window, image=self.background_photo)
...
class TicTacToe:
    def __init__(self, main_menu_window):
        ...
        self.buttons_frame = ttk.Frame(self.game_window)
        ...
        for i in range(3):
            row = []
            for j in range(3):
                button = ttk.Button(self.buttons_frame, text=" ", width=10, command=lambda x=i, y=j: self.make_move(x, y))
